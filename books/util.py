...
#if we're using the half-ratings test/train data, calculates an RMSE given predictions
def rmse(preds):
    parsed_preds = {}

    for row in preds:
        parsed_preds[row["user"]] = row["rating"]

    ratings = []

    with open("half-ratings-test.csv", 'rb') as fh:
        reader = csv.reader(fh)
        header = next(reader, None)        
        for row in reader:
            ratings.append( { 'id':     int(row[0]),
    ...
    for row in ratings:
        ...
        predicted_rating = parsed_preds[user]
        residual = actual_rating - predicted_rating
        residuals.append(residual)

        error_sum = error_sum + np.square(residual)

    #plot(actuals, residuals)

    return np.sqrt(1.0 / len(ratings) * error_sum)

#can plot an actual vs residual scatterplot if need be
def plot(actual, residual):
    fig = figure()
    ax = fig.add_subplot(111)
 
    ax.set_title("Actual vs Residual Plot")
    ax.set_xlabel("Actual")
    ax.set_ylabel("Residual")

    col = ax.scatter(actual, residual, 50)

    show()

#helper function to decipher city state nation
def citystatenation(string):
    parsed = string.split(",")
    if len(parsed) == 3:
        return {"city": parsed[0], "state": parsed[1], "nation": parsed[2]}
    else:
        return False
